...
type Connection = { connection_id: string; agent_id: string; endpoint: string; adapter_metadata: { protocol_version?: string; environment?: string }; created_at: string };
type Run = { run_id: string; agent_id: string; connection_id: string; status: "queued" | "running" | "completed" | "failed"; current_round: number; last_event_seq: number; report_available: boolean; error_message?: string | null; selected_seed_ids: string[]; config: { rounds: number; seed_count: number; variants_per_seed: number } };
type RunEvent = { event_id: string; run_id: string; seq: number; timestamp: string; type: string; payload: Record<string, unknown> };
type Report = { run_id: string; conclusion: "exposure_confirmed" | "coverage_incomplete" | "no_bypass_observed"; outcome_summary: Record<string, number>; strategy_effectiveness?: Array<{ strategy: string; variants: number; bypasses: number; weight_final: number }>; round_evolution?: Array<{ round: number; variants: number; active_strategies: string[] }>; weight_snapshots?: Array<{ round: number; weights_before: Record<string, number>; weights_after: Record<string, number>; active_strategies: string[] }>; bypasses?: Array<{ variant_id: string; seed_id: string; strategy: string; risk_pattern: string }>; seeds_used?: Array<{ id: string; risk_pattern: string; severity: string }> };
...
export function RedTeamWorkspace({ activeAgentId }: { activeAgentId: string }) {
  ...
  useEffect(() => { void refresh().catch((cause: unknown) => setError(cause instanceof Error ? cause.message : "读取红队资源失败")); }, [activeAgentId]);
  ...
  return <section className={`redteam-page is-${view}`} aria-label="红队演练工作区">
    <header className="redteam-header"><div><span>ADAPTIVE RED TEAM</span><h1>{view === "entry" ? "红队演练" : view === "running" ? "演练进程" : "演练报告"}</h1><p>{view === "entry" ? "固定种子集，按每轮权重最高的三种策略持续变异与验证。" : activeRun ? `${activeRun.agent_id} · ${activeRun.run_id.slice(-8)}` : "运行专属证据与覆盖度结论"}</p></div><div className="redteam-header-actions">{view !== "entry" && <button type="button" className="redteam-ghost" onClick={() => setView("entry")}>返回入口</button>}<button type="button" className="redteam-ghost" onClick={() => void refresh()}>刷新历史</button></div></header>
