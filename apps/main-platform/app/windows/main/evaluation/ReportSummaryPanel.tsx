import type { RiskFinding, ReportSummary } from "./evaluation-types";
import { buildRiskPatternRows, buildSummaryRows, type SummaryRow } from "./report-summary";
import { buildRiskCoverage } from "./report-topology-summary";
...
function CoverageBlock({ summary, findings }: { summary: ReportSummary; findings: RiskFinding[] }) {
  const coverage = buildRiskCoverage(summary, findings);
  return <section className="evaluation-risk-coverage" aria-label="风险覆盖摘要"><header><span className="evaluation-eyebrow">RISK COVERAGE</span><span>按报告 summary 统计</span></header>{coverage.map((group) => <div className="evaluation-risk-coverage-row" key={group.key}><strong>{group.label}</strong>{group.available ? <span>{group.total === 0 ? "0/0 未启用" : `${group.tested}/${group.total} 已测试`}</span> : <span className="is-unavailable">覆盖统计不可用</span>}</div>)}</section>;
}

export function ReportSummaryPanel({ summary, findings = [] }: { summary: ReportSummary; findings?: RiskFinding[] }) {
  return <section className="evaluation-summary-panel evaluation-report-reveal" aria-label="批量测评统计摘要"><header className="evaluation-summary-heading"><div><span className="evaluation-eyebrow">BATCH SUMMARY</span><h2>批量统计</h2></div><div className="evaluation-summary-overall"><span><b>{summary.total_tests}</b> 总数</span><span className="is-pass"><b>{summary.passed}</b> PASS</span><span className="is-fail"><b>{summary.failed}</b> FAIL</span><span className="is-error"><b>{summary.error}</b> ERROR</span><strong>{Math.round(summary.pass_rate * 1000) / 10}%</strong></div></header><div className="evaluation-summary-legend"><span className="is-pass">PASS</span><span className="is-fail">FAIL</span><span className="is-error">ERROR</span></div><div className="evaluation-summary-grid"><DimensionChart title="R1–R4 / OTHER" rows={buildRiskPatternRows(summary.by_risk_pattern)} /><DimensionChart title="RISK TYPE" rows={buildSummaryRows(summary.by_risk_type)} /><DimensionChart title="SEVERITY" rows={buildSummaryRows(summary.by_severity, SEVERITY_ORDER)} /></div><CoverageBlock summary={summary} findings={findings} /></section>;
}
