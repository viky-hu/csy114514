...
export function EvaluationReportWorkspace({ onNavigate }: { onNavigate?: EvaluationWorkspaceNavigate }) {
  ...
  return <section ref={root} className={`evaluation-page evaluation-report-page ${report.summary ? "has-summary" : ""}`} aria-label="测评报告工作台"><header className="evaluation-page-header evaluation-report-reveal"><div><span className="evaluation-eyebrow">EVALUATION REPORT</span><h1>测评报告</h1><p>{report.conclusion}</p></div><div className="evaluation-report-actions"><EvaluationAgentBadge agentId={report.agent_id} />{topologyResult?.topology ? <span className="evaluation-report-topology-badge" title={topologyResult.errorMessage ?? "当前 Agent 拓扑"}><span>{topologyLabel(topologyResult.topology)}</span><small>{topologyResult.topology.nodes.length} 节点 · {topologyResult.topology.edges.length} 通道</small></span> : topologyResult ? <span className="evaluation-report-topology-badge is-unavailable" title={topologyResult.errorMessage ?? "Topology backend is unavailable"}><span>拓扑不可用</span><small>未将示例图谱作为已确认拓扑</small></span> : null}<button type="button" className="evaluation-secondary-button" onClick={() => onNavigate?.("run")}><ArrowLeft size={15} />返回运行</button><span className={`evaluation-severity-badge is-${severityClass(report.severity)}`}>{SEVERITY_LABEL[report.severity] ?? report.severity}</span></div></header><div className="evaluation-report-reveal"><ScorePanel report={report} /></div>{report.summary && <ReportSummaryPanel summary={report.summary} findings={findings} />}<div className={`evaluation-report-layout evaluation-report-reveal ${drawerOpen ? "is-drawer-open" : ""}`}><FindingList findings={findings} selectedId={selected?.finding_id} onSelect={(finding) => { setSelectedId(finding.finding_id); setDrawerOpen(true); }} />{selected ? <EvidenceDetail finding={selected} traceEvents={(trace?.events ?? []) as SequencedEvent[]} onClose={() => setDrawerOpen(false)} /> : <div className="evaluation-empty-report"><CheckCircle2 size={24} /><h2>没有已确认的风险发现</h2><p>本次运行没有返回可复算的 Judge Finding。</p></div>}</div></section>;
}
