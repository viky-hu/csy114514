...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    const upstream = await fetch(buildAgentEvalBackendUrl(`/evaluations/comparisons/${encodeURIComponent(comparisonId)}/report`), { headers: { Accept: "application/json" }, cache: "no-store", signal: request.signal });
