...
## 2.2 SR0 字段命名决议(2026-08-03 三人确认)

以下字段名在 SR0 串跑中对齐确认, 所有模块必须遵守。

### AttackGraph 节点与边

```text
节点 ID:         node_id          (不是 id)
节点类型:        node_type        (不是 type)
节点标签:        labels
节点元数据:      metadata

边 ID:           edge_id          (不是 id)
边源节点:        source_node_id   (不是 source)
边目标节点:      target_node_id   (不是 target)
边类型:          edge_type        (不是 type)
边元数据:        metadata
...
所属图:          graph_id
节点序列:        node_ids
边序列:          edge_ids         (Security Line 扩展, R3 确认)
跳数:            hop_count        (Security Line 扩展, R3 确认)
风险模式:        risk_pattern_id
风险类型:        risk_type
严重等级:        severity
描述:            description      (不是 reason)
```

### AgentManifest

```text
工具能力:        capabilities: list[str]
工具权限:        tool_permissions: dict[str, str]   (tool_name → ALLOW|CONFIRM|DENY)
```

> Stage 1 使用 capabilities + tool_permissions 分离方案(字典映射)。
> design V0.2 §4 示例中的 tools 数组方案不采用。
> 若 Stage 2 需统一为 tools 数组, 须走 §4 变更流程。
...
# 9. 项目仓库与实际目录结构(2026-08-03 SR0 补充)

## 9.1 仓库地址

```text
https://github.com/viky-hu/csy114514
```

## 9.2 实际目录 vs design V0.2 计划

design V0.2 §2 规划了理想目录, 实际落地时因前端采用 pnpm monorepo 而有调整:

```text
design V0.2 计划              实际 (repo)                     说明
─────────────               ──────────                     ────
backend/          →         csy——全智赛/backend/            Platform + Security 后端
frontend/         →         apps/main-platform/             Next.js 前端 (pnpm workspace)
shared/contracts/ →         csy——全智赛/shared/contracts/   唯一 source of truth
shared/fixtures/  →         csy——全智赛/shared/fixtures/    Mock API 数据源
shared/examples/  →         csy——全智赛/shared/examples/    Security Line 资产
docs/             →         csy——全智赛/docs/               项目文档
tests/            →         csy——全智赛/backend/tests/      后端测试
reference-agent/  →         (Stage 2)                       CorpMate 尚未落库
```

> **注意:** 根目录也有一份 `shared/contracts/`, 是早期遗留。
> 以 `csy——全智赛/shared/contracts/` 为准, 根目录那份将删除(L2 决议)。

## 9.3 Security Line 资产位置

```text
csy——全智赛/shared/examples/security/
  ├── risk_patterns.json          S0-1: 4 个 RiskPattern (R1-R4)
  ├── attack_seeds.json           S0-4: 8 个 AttackSeed
  └── security_testcases.json     S0-3: 6 个 TestCase

csy——全智赛/docs/
  └── security-model.md           S0-2: 攻击语义文档

csy——全智赛/shared/contracts/
  ├── SECURITY_CONTRACTS.md       Contract 法律文档 (Security Line 三契约)
  ├── risk_pattern.schema.json    RiskPattern JSON Schema
  ├── attack_seed.schema.json     AttackSeed JSON Schema
  └── test_case.schema.json       TestCase JSON Schema
```

## 9.4 AI Agent 开工补充检查

在 §5 Before Coding 的基础上, 补充:

```text
□ clone repo 并确认最新 commit
□ 读完 shared/contracts/SECURITY_CONTRACTS.md
□ 确认本任务的代码在 csy——全智赛/ 下的哪个子目录
□ 不在根目录创建 backend/ 或 frontend/ (它们在 csy——全智赛/ 和 apps/ 下)
```
